...
public void GrabarAmarillosSinRepetir(Object[][] mat, int MaxF, int MaxC, CRUDVehiculo crud, Archivos arch) {
    ...
    for (int i = 0; i < MaxF; i++) {
        for (int j = 0; j < MaxC; j++) {
            if (mat[i][j] != null) {
                objV = (Vehiculos) mat[i][j];
                if (objV.getColor().equalsIgnoreCase("Amarillo")) {
                    if (!crud.Buscar(arch, objV.getNroPlaca())) {
                        crud.GrabarVehiculo(arch, objV);
                        seGrabo = true;
                    }
                }
            }
    ...
    if (seGrabo) {
        JOptionPane.showMessageDialog(null, "Vehículos amarillos grabados correctamente en el archivo.");
    } else {
        JOptionPane.showMessageDialog(null, "No se grabaron vehículos (ninguno amarillo o ya estaban grabados).");
    }
}

public void mostrarMatriz(Vehiculos[][] matriz) {
        for (Vehiculos[] matriz1 : matriz) {
            for (Vehiculos item : matriz1) {
                if (item != null) {
                    System.out.print("[" + item.getNroPlaca() + "] ");
                } else {
